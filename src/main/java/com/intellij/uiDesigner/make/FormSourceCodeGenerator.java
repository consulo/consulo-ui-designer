...
public final class FormSourceCodeGenerator
{
		...
		FormEditingUtil.iterate(rootContainer, new FormEditingUtil.ComponentVisitor<LwComponent>()
		{
			public boolean visit(final LwComponent iComponent)
			{
				...
				if(errorInfo != null)
				{
					...
					{
						message = UIDesignerBundle.message("error.for.component", iComponent.getBinding(),
								errorInfo.myDescription);
					}
					else
					{
			...
			final Module module) throws CodeGenerationException, IncorrectOperationException
	{
		...
		if(classToBind == null)
		{
			throw new ClassToBindNotFoundException(UIDesignerBundle.message("error.class.to.bind.not.found",
					rootContainer.getClassToBind()));
		}
		...
		final PsiElementFactory elementFactory = JavaPsiFacade.getInstance(psiManager.getProject())
				.getElementFactory();
		...
		final PsiClass fakeClass = elementFactory.createClassFromText("{\n" +
				"// GUI initializer generated by " + ApplicationNamesInfo.getInstance().getFullProductName() + " GUI " +
				"Designer\n" +
				"// >>> IMPORTANT!! <<<\n" +
				...
				"}\n" +
				"\n" +
				"/** Method generated by " + ApplicationNamesInfo.getInstance().getFullProductName() + " GUI " +
				"Designer\n" +
				" * >>> IMPORTANT!! <<<\n" +
		...
		for(PsiMethod constructor : newClass.getConstructors())
		{
			...
			else if(haveCustomCreateComponents && hasCustomComponentAffectingReferences(constructor, newClass,
					rootContainer, null))
			{
				needInitializer = false;
				needSetupUI = true;
		...
		generateMethodIfRequired(newClass, method, AsmCodeGenerator.GET_ROOT_COMPONENT_METHOD_NAME, grcMethodText,
				topComponent.getBinding() != null);

		final String loadButtonTextMethodText = getLoadMethodText(AsmCodeGenerator.LOAD_BUTTON_TEXT_METHOD,
				AbstractButton.class, module);
		generateMethodIfRequired(newClass, method, AsmCodeGenerator.LOAD_BUTTON_TEXT_METHOD, loadButtonTextMethodText,
				myNeedLoadButtonText);
		final String loadLabelTextMethodText = getLoadMethodText(AsmCodeGenerator.LOAD_LABEL_TEXT_METHOD,
				JLabel.class, module);
		generateMethodIfRequired(newClass, method, AsmCodeGenerator.LOAD_LABEL_TEXT_METHOD, loadLabelTextMethodText,
				myNeedLoadLabelText);
			...
			final PsiMethod setupUIMethod)
	{
		...
		for(PsiStatement statement : statements)
		{
			...
			if(!ourSuperCallPattern.accepts(statement) && hasCustomComponentAffectingReferences(statement,
					classToBind, rootContainer, callsThisConstructor))
			{
				anchor = statement;
				break;
			}
		...
		if(!callsThisConstructor.get().booleanValue())
		{
			...
			try
			{
				PsiStatement setupUIStatement = factory.createStatementFromText(AsmCodeGenerator.SETUP_METHOD_NAME + "" +
						"();", constructor);
	...
	private String getLoadMethodText(final String methodName, final Class componentClass, Module module)
	{
		...
		return "/** @noinspection ALL */ " +
				"private void " + methodName + "(" + componentClass.getName() + " component, " +
				...
				"  if (haveMnemonic) {" +
				(componentClass.equals(AbstractButton.class) ? "        component.setMnemonic(mnemonic);" : "        " +
			...
			final PsiClass aClass) throws CodeGenerationException
	{
		...
		final String componentClass = component instanceof LwNestedForm ? getNestedFormClass(module,
				(LwNestedForm) component) : getComponentLayoutGenerator(component.getParent()).mapComponentClass
				(component.getComponentClassName());

		if(component.isCustomCreate() && component.getBinding() == null)
		{
			throw new CodeGenerationException(component.getId(), UIDesignerBundle.message("error.custom.create.no" +
					".binding"));
		}
		...
		for(final LwIntrospectedProperty property : introspectedProperties)
		{
			...
			final boolean isTextWithMnemonicProperty = "text".equals(property.getName()) && (isAssignableFrom
					(AbstractButton.class.getName(), componentClass, globalSearchScope) || isAssignableFrom(JLabel
		...
		if(!(component.getParent() instanceof LwRootContainer))
		{
			...
			getComponentLayoutGenerator(component.getParent()).generateComponentLayout(component, this, componentVar,
					parentVariable);
		}
		...
		if(component instanceof LwContainer)
		{
			...
			{
				generateSetupCodeForComponent((LwComponent) container.getComponent(i), component2TempVariable,
						class2variableIndex, id2component, module, aClass);
			}
		}
	}
			...
			final PsiClass aClass)
	{
		...
		if(component instanceof LwContainer)
		{
			...
			for(int i = 0; i < container.getComponentCount(); i++)
			{
				generateComponentReferenceProperties((LwComponent) container.getComponent(i), component2variable,
						class2variableIndex, id2component, aClass);
			}
		}
	}
			...
			final PsiClass aClass)
	{
		...
		@NonNls final String className = component instanceof LwNestedForm ? "nestedForm" : component
