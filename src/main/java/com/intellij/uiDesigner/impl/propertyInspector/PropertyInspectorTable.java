...
import com.intellij.uiDesigner.impl.actions.ShowJavadocAction;
...
public final class PropertyInspectorTable extends Table implements DataProvider
{
	...
	PropertyInspectorTable(Project project, @Nonnull final ComponentTree componentTree)
	{
		...
		// Popup menu
		PopupHandler.installPopupHandler(this, (ActionGroup) ActionManager.getInstance().getAction(IdeActions
	...
	public PsiClass getComponentClass()
	{
		...
		return JavaPsiFacade.getInstance(module.getProject()).findClass(className, GlobalSearchScope.moduleWithDependenciesAndLibrariesScope
				(module));
	}
	...
	public void synchWithTree(final boolean forceSynch)
	{
		...
		try
		{
			...
			for(int i = reversePath.size() - 1; i >= 0; i--)
			{
				...
				{ // try to expand parent and try again
	...
	private void startEditing(final int index)
	{
		...
		{ // use default policy to find preferred focused component
	...
	private static void showInvalidInput(final Exception exc)
	{
		...
		Messages.showMessageDialog(UIDesignerBundle.message("error.setting.value", message), UIDesignerBundle.message("title.invalid.input"),
