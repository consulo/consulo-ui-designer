...
public abstract class AsmCodeGeneratorTest extends TestCase
{
	...
	private AsmCodeGenerator initCodeGenerator(final String formFileName, final String className, final String testDataPath) throws Exception
	{
		...
		try (StandardJavaFileManager manager = compiler.getStandardFileManager(listener, Locale.US, StandardCharsets.UTF_8))
		{
			manager.setLocation(StandardLocation.CLASS_OUTPUT, Collections.singleton(new File(tmpPath)));

			compiler.getTask(null, manager, listener, null, null, manager.getJavaFileObjects(new File(javaPath))).call();
		}
    ...
    */
	...
	public void testClientProp() throws Exception
	{  // IDEA-46372
